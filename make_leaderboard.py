...
from compare import load_categories, load_summaries  # noqa: E402
...
def category_section(cats: dict) -> str:
    """Second table: quality rate per prompt category, with schema validity beneath."""
    if not cats:
        return ""
    categories = sorted({c for by_cat in cats.values() for c in by_cat})
    head = "".join(f"<th>{c.replace('-', ' ')}</th>" for c in categories)
    body = []
    for model, by_cat in cats.items():
        ...
        for cat in categories:
            ...
            if not c:
                cells.append("<td>–</td>")
                continue
            q = pct(c["quality_rate"])
            cells.append(
                f"<td>{q}<span class='sub2'>schema {pct(c['schema_valid_first_try_rate'])}</span></td>"
            )
        body.append(f"<tr><td class='model'>{model}</td>{''.join(cells)}</tr>")
    return f"""
  <h2>Where the answers actually fail</h2>
  <p class="sub">Answer quality per prompt category (first-try schema validity beneath).
     Schema validity holds up across categories; answer quality does not.</p>
  <div class="tablewrap">
  <table>
    <thead><tr><th class="model">Model</th>{head}</tr></thead>
...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; }}
  ...
  td.model, th.model, #lb th:nth-child(2) {{ text-align: left; font-weight: 600; }}
  td.hero {{ color: var(--accent); font-weight: 700; background: var(--accent-soft); }}
  h2 {{ font-size: 1.15rem; letter-spacing: -0.01em; margin: 2.4rem 0 0; }}
  .sub2 {{ display: block; color: var(--muted); font-size: 0.76rem; font-weight: 400; }}
...
<main>
  ...
  {category_section(load_categories())}
